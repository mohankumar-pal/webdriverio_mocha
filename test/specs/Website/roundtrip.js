...
const managebooking = require('../../../src/website/pageobjects/OnewayRoundtrip/managebookingpo.js')
...
let OD = JSON.parse(fs.readFileSync('test/testdata/json/website/RTandMB.json'))
            ...
            if (await ExecutionFlag === "Yes") {
                try {
                    ...
                    if (ModifySearch == "Yes") {
                        ...
                        if (ChangeDate == 'Yes') {
                            await flightlist.mscalendar(DepartureDate2, ReturnDate2)
                        }
                        if (ChangePax == "Yes") {
                    ...
                    //ORG DEST VALIDATION
                    await validate.RTfromtovalidation(From, To)

                    //DATE VALIDATION
                    await validate.RTdeparturedatevalidation(Departuredate)
                    await validate.RTreturndatevalidation(Returndate)
                    ...
                    if (ReturnCabinType == "First") {
                        var cab1index = flightlistpo.optionFirstCabin
                    }
                    else if (ReturnCabinType == "Business") {
                        var cab1index = flightlistpo.optionBusinessCabin
                    }
                    else if (ReturnCabinType == "Economy") {
                        var cab1index = flightlistpo.optionEconomyCabin
                    }
                    else if (ReturnCabinType == "Premium Economy") {
                        var cab1index = flightlistpo.optionPremiumCabin
                    }
                    await flightlist.flightselection(FlightType, cab1index)
                    await flightlist.cabinselection(ReturnFlightSubCabinType)
                    ...
                    await library.pagescrolltomiddleAction(addservicepo.textAddService)
                    //await browser.pause(3000)
                    if (SeatType != "") {
                        if (await library.elementdisplayedAction(addservicepo.buttonSelectSeat)) {
                            ...
                            if (SeatType == "FreeSeat") {
                                ...
                                var seatconfirm = addservicepo.buttonExtrallegroomconfirm
                            }
                    ...
                    //PAYMENT
                    await paymentpo.textPromocode.waitForExist({ timeout: 200000 })
                    await browser.pause(4000)
                    /* if (Promocode != "") {
                    ...
                    fs.writeFileSync('./test/outputartifact/result/website/RoundTripResult.xlsx', xls, 'binary');
                    //await browser.pause(3000)
