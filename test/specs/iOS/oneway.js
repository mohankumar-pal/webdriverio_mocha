    ...
    it('One Way', async () => {
      if (ExecutionFlag == "Yes") {
        ...
        try {
          ...
          await flightlist.flcabinselection(CabinType, SubCabinType, DoYouWanttoUpgrade, SearchUsingFlightCode, FlightCode)
          //PASSENGER DETAILS
          ...
          var xls = tool.json2xls(finaldata, { order: ["TestcaseScenario", "PNR", "From", "To", "DateandTime", "PromoCode", "Currency", "TestcaseResult"] });
          fs.writeFileSync('./test/outputartifact/result/mobile/OneWayResult.xlsx', xls, 'binary');
        ...
        if (ManageBooking == "Yes" && testexecutionresult != "Failed") {
          ...
          // await library.clickAction(oneway.textMbPassenger)
          // for (var i = 0; i < NumberofAdult; i++) {
          //   await validate.paxnamevalidation(adultfirstname[i], adultlastName[i])
          // }
          // for (var i = 0; i < NumberofChild; i++) {
        ...
        if (Checkin == "Yes" && testexecutionresult != "Failed") {
          ...
          // //Date Validation
          // if (ManageBooking == "Yes" && ModifyFlights == "Yes") {
          //   await library.waituntilelementexistAction(checkinpo.buttonCheckin)
          //   var departuredate = Segment1Date
          //   var datemonth = departuredate.slice(0, 6)
          //   const mbdate = '//XCUIElementTypeStaticText[contains(@name,"' + datemonth + '")]'
          //   const mbdateloc = await $(mbdate)
          //   await expect(mbdateloc).toBeExisting()
          // }
          ...
          let checkinjsonresult = await checkin.checkin(Testcase, PNR, Adult1LastName, Checkin, EmailId, CkCountryCode, PhoneNumber, CkNationality, CkCountry, AdtGender,
            AdtDOB, CkDocumentType, CkDocumentNumber, CkDocExpDate, CkDocIssuingCountry, CkHouseAddress, CkVisaNumber, CkVisaCountry, CkVisaIssueDate, CkVisaExpDate, CkVisaIssuingCountry, CkPurposeOfVisit, SendBySMS, SendAsPdf, SendPassBook, FFPProgram, FFPNumber, timestamp, CkNumberofAdult, CkNumberofChild,
          ...
          //adultfirstname, adultlastName, childfirstname, childlastName, infantfirstname, infantlastname, NumberofAdult, NumberofChild, NumberofInfant)
