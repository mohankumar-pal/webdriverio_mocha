...
const OD = JSON.parse(fs.readFileSync("test/testdata/json/mobile/Multicity.json"))
    ...
    it('Multicity', async () => {
      if (ExecutionFlag == "Yes") {
        try {
          ...
          var Nationality = [Adult1Nationality, Adult2Nationality, Adult3Nationality, Adult4Nationality, Adult5Nationality, Adult6Nationality, Adult7Nationality, Adult8Nationality, Adult9Nationality]
          ...
          var adultexpirydate = [Adult1Expirydate, Adult2Expirydate, Adult3Expirydate, Adult4Expirydate, Adult5Expirydate, Adult6Expirydate, Adult7Expirydate, Adult8Expirydate, Adult9Expirydate]
          var adultexpirymonth = [Adult1Expirymonth, Adult2Expirymonth, Adult3Expirymonth, Adult4Expirymonth, Adult5Expirymonth, Adult6Expirymonth, Adult7Expirymonth, Adult8Expirymonth, Adult9Expirymonth]
          var adultexpiryyear = [Adult1ExpiryYear, Adult2ExpiryYear, Adult3ExpiryYear, Adult4ExpiryYear, Adult5ExpiryYear, Adult6ExpiryYear, Adult7ExpiryYear, Adult8ExpiryYear, Adult9ExpiryYear]
          ...
          var childexpirydate = [Child1Expirydate, Child2Expirydate, Child3Expirydate, Child4Expirydate, Child5Expirydate, Child6Expirydate, Child7Expirydate, Child8Expirydate, Child9Expirydate]
          var childexpirymonth = [Child1Expirymonth, Child2Expirymonth, Child3Expirymonth, Child4Expirymonth, Child5Expirymonth, Child6Expirymonth, Child7Expirymonth, Child8Expirymonth, Child9Expirymonth]
          var childexpiryyear = [Child1ExpiryYear, Child2ExpiryYear, Child3ExpiryYear, Child4ExpiryYear, Child5ExpiryYear, Child6ExpiryYear, Child7ExpiryYear, Child8ExpiryYear, Child9ExpiryYear]
          ...
          var infantexpirydate = [Infant1Expirydate, Infant2Expirydate, Infant3Expirydate, Infant4Expirydate, Infant5Expirydate, Infant6Expirydate, Infant7Expirydate, Infant8Expirydate, Infant9Expirydate]
          var infantexpirymonth = [Infant1Expirymonth, Infant2Expirymonth, Infant3Expirymonth, Infant4Expirymonth, Infant5Expirymonth, Infant6Expirymonth, Infant7Expirymonth, Infant8Expirymonth, Infant9Expirymonth]
          var infantexpiryyear = [Infant1ExpiryYear, Infant2ExpiryYear, Infant3ExpiryYear, Infant4ExpiryYear, Infant5ExpiryYear, Infant6ExpiryYear, Infant7ExpiryYear, Infant8ExpiryYear, Infant9ExpiryYear]
          ...
          for (var i = 0; i < NumberofAdult; i++) {
            ...
            if (!await library.elementdisplayedAction(paxdetails.textErrors)) {
              await passenger.mcbasicdetails(adulttittle[i], adultfirstname[i], adultlastName[i], gender[i], adultdobday[i], adultdobmonth[i], adultdobyear[i], Nationality[i], passportno[i], passportissuecountry[i], adultexpirydate[i], adultexpirymonth[i], adultexpiryyear[i], ffp[i], frequentpairlineadult[i], frequentflyernumber[i])
              //intpassport = await passenger.mcpassportdet(travelerinfo[i], adultfirstname[i], adultlastName[i], gender[i], Nationlaity[i], passportno[i], expirydate[i], passportissuecountry[i], ffp[i], frequentpairlineadult[i], frequentflyernumber[i])
              ...
              if (NumberofInfant != 0 && j < NumberofInfant) {
                //for (i = 0; i < NumberofInfant; i++) {
                await library.waituntilelementexistAction(paxdetails.textboxMcFirstName)
                await passenger.mcinfantbasicdetails(infantfirstname[j], infantlastname[j], infantdobday[j], infantdobmonth[j], infantdobyear[j], infantgender[j], infantnationality[i], infantpassportno[i], infantexpirydate[i], infantexpirymonth[i], infantexpiryyear[i], infantpassportissuecountry[i])
                j++
          ...
          for (i = 0; i < NumberofChild; i++) {
            ...
            if (!await library.elementdisplayedAction(paxdetails.textErrorMsg)) {
              await passenger.mcchildbasicdetails(childtittle[i], childfirstname[i], childlastName[i], childdobDay[i], childdobMonth[i], childdobYear[i], childgender[i], childnationality[i], childpassportno[i], childexpirydate[i], childexpirymonth[i], childexpiryyear[i], childpassportissuecountry[i], childffp[i], childfairline[i], childfairlinenumber[i])
          ...
          if (DoyouwanttoaddSeat == "Yes") {
            ...
            //await driver.switchContext('WEBVIEW_4036.5')
            await library.waituntilelementexistAction(addservice.textTravellers)

            //await browser.pause(9000)
            // if (SeatType == "FreeSeat") {
            //   var seatindex = addservice.buttonMcFreeSeat
            // }
            // else if (SeatType == "PreferredSeat") {
            //   var seatindex = addservice.buttonMcPreferredSeat
            // }
            // else if (SeatType == "ExtralegroomSeat") {
            //   var seatindex = addservice.buttonMcExtralegroomseat
            // }
            // else if (SeatType == "BusinessClassSeat") {
            //   var seatindex = addservice.buttonBusinessClassSeat
            // }
            //await addionalservice.Mcseatselection(seatindex)
            await addionalservice.Mcnextflight(SeatType)

            await browser.pause(2000)
            await addionalservice.McConfirmseat()
          ...
          if (DouyouwanttoaddAB === "Yes") {
            await browser.pause(3000)
            if (await library.elementexistingAction(addservice.textBaggage)) {
              await library.clickAction(addservice.textBaggage)
              await browser.pause(3000)

              await library.waituntilelementexistAction(addservice.textBaggage)
          ...
          if (DoyouwanttoaddWC === "Yes") {
            ...
            await addionalservice.multicitywheelchairslctseats(SeatType)
            await library.waituntilelementexistAction(oneway.textAdditionalServices)
          ...
          //PAYMENT PAGE  //tested above modules in iOS prod build so haven't tested below payment code
          
          await library.waituntilelementexistAction(validation.textMcPromoCode)
          await browser.pause(9000)
          //var promotxt = await validate.promocodepaymentpagevalidation(PromoCodeAtPaymentPage, Promocode)
          if (await library.elementdisplayedAction(validation.textMcPromoCode)) {
            if (PaymentType === "CreditCard") {
              await payment.McCreditCardPayment(CardNumber, CardExpMonth, CardExpYear, CVV, CardHolderName, CardEmail, CardMobile, HouseNumber, ApartmentNumber, Pincode, City, Country, State, CdEmail, CdMobNum)
            }
          ...
          fs.writeFileSync('./test/outputartifact/result/mobile/MulticityResult.xlsx', xls, 'binary');
