...
const OD = JSON.parse(fs.readFileSync("test/testdata/json/RoundTrip.json"))
    ...
    it('Round Trip', async () => {
      if (ExecutionFlag == "Yes") {
        try {
          ...
          await flightlist.flcabinselection(OnwardCabinType, SubCabinType, DoYouWanttoUpgrade) //onward
          await library.waituntilelementexistAction(oneway.buttonModifySearch)
          await browser.pause(3000)
          await flightlist.flcabinselection(ReturnCabinType, ReturnFlightSubCabinType, DoYouWanttoUpgrade) //return
          await library.waituntilelementexistAction(oneway.buttonModifySearch)
          await browser.pause(3000)
          //PASSENGER DETAILS
          ...
          for (var i = 0; i < NumberofAdult; i++) {
            ...
            if (!await library.elementdisplayedAction(paxdetails.textErrorMsg)) {
              ...
              if (i == 0) {
                if (IsConcessionary == "No" && await intpassport == 0) {
                  await passenger.basiccontact(Adult1Email, Adult1Email, CountryCode, MobileNumber)
                  await passenger.emergency(EmergencyContact, EmergencyContactName, EmergencyCountryCode, EmergencyContactMobileno,)
                }
                else if (IsConcessionary == "Yes" && await intpassport == 0) {
                  await passenger.conbasiccontact(Adult1Email, Adult1Email, CountryCode, MobileNumber)
                  await passenger.conemergency(EmergencyContact, EmergencyContactName, EmergencyCountryCode, EmergencyContactMobileno,)
                }
              ...
              if (await library.elementexistingAction(paxdetails.buttonNextPassenger))
          ...
          if (SelectSeat == "Yes") {
            ...
            await additionalservice.seatselection(seatindex, seatconfirm)
            await additionalservice.nextflight(SeatType)
            await additionalservice.confirmseat()
            await library.waituntilelementexistAction(oneway.textYourflights)
          }
          ...
          if (DoyouwanttoaddAB === "Yes") {
            ...
            if (await addService.addExtraBaggage.isDisplayed()) {
              ...
              await additionalservice.addbaggage(DomDoyouwanttoaddAB, paxAdditionalBaggage, baggageOption, changeflag)
              ...
              if (DomChangeAdditionalBaggage === "Yes") {
                if (await addService.changeAB.isDisplayed()) {
                  ...
                  changeflag = 1
                  await additionalservice.changebaggage(paxAdditionalBaggage, baggageOption, changeflag)
          ...
          if (DomDoyouwanttoaddWhcr === "Yes") {
            await additionalservice.wheelchair()
            await additionalservice.wheelchairslctseats(SeatType)
            await library.waituntilelementexistAction(oneway.textYourflights)
          ...
          if (Meals === "Yes") {
            ...
            await additionalservice.selectmeals(mealspax)
          }
          await additionalservice.checkout()
          //PAYMENT PAGE
          ...
          fs.writeFileSync('./test/outputartifact/result/RoundTripResult.xlsx', xls, 'binary');
